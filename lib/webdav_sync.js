...
var exec, util, colors, fs, watch, klaw; //libraries
watch = require("watch");
fs = require("fs");
...
module.exports = function (options) {
  ...
  return {
    start: function() {
      ...
      if (options.upload_once) {
        console.log('Will Upload once and then exit.');

        klaw(options.local_base)
            .on('data', function cb(item) {
              var path = item.path.replace(cwd + '/', '');
              created(path, item.stats);
